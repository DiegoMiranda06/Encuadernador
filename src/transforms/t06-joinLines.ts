 ...
 */
function isParagraphBreak(prev: IRLine, next: IRLine, medianGap: number, blockRightEdge: number): boolean {
  const gap = next.bbox[1] - prev.bbox[3]
  if (medianGap > 0 && gap > medianGap * GAP_FACTOR) return true
...
function joinBlockLines(lines: IRLine[], medianGap: number): { lines: IRLine[]; changed: number } {
  ...
  for (let i = 1; i < lines.length; i++) {
    const next = lines[i]
    if (isParagraphBreak(current, next, medianGap, rightEdge)) {
      result.push(current)
      current = next
    } else {
 ...
 */
export const t06JoinLines: Transform = (pages) => {
  ...
  const outputPages: IRPage[] = pages.map((page) => ({
    ...
    blocks: page.blocks.map((block) => {
      ...
      const { lines, changed: blockChanged } = joinBlockLines(block.lines, medianLineGap)
